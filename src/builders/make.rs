use crate::{builders::builder_trait::BuilderImpl, cli::child_logger};
...
impl Make {
    ...
    >(
        ...
        if !result.success() {
            return Err(format!(
                "Failed to execute {configure:?}. Output:\n{}\n{}",
                stdout.join("\n"),
                stderr.join("\n")
            ));
        }
...
impl BuilderImpl for Make {
    ...
    >(
        ...
        self.configure(source_path, build_path, install_path)?;
